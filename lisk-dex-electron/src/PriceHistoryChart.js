...
const TimeAxisTick = ({ x, y, stroke, payload }) => {
	return (
...
class PriceHistoryChart extends React.PureComponent {
	...
	render() {
		...
		let maxVolume = this.props.data.reduce((accumulator, entry) => {
			return entry.volume > accumulator ? entry.volume : accumulator;
		}, -Infinity);
		let maxPrice = this.props.data.reduce((accumulator, entry) => {
			return entry.price > accumulator ? entry.price : accumulator;
		}, -Infinity);

		let assetSymbol = this.props.assets[1].toUpperCase();
		...
		return (
	      ...
	      >
	        ...
	        <YAxis label={{ value: `Price (${assetSymbol})`, angle: -90, fill: '#999999', dx: -25 }} />
